...
class ShowImageItemNewsAndNotification extends StatefulWidget {
  final String urlImage;

  const ShowImageItemNewsAndNotification({
    Key key,
    this.urlImage,
  }) : super(key: key);

  @override
  _ShowImageItemNewsAndNotificationState createState() =>
      _ShowImageItemNewsAndNotificationState();
}

class _ShowImageItemNewsAndNotificationState
    extends State<ShowImageItemNewsAndNotification> {
  AppBloc appBloc;
...
//                ),
  ...
  void _onDownloadImage() async {
    if (!isClickedDownload) {
      ...
      String linkDownload = widget.urlImage;
      if (Platform.isAndroid) {
        Downloader downloader = await Downloader.init();
        TaskInfo taskInfo = TaskInfo(
            linkDownload, appBloc.orientBloc.getNameFileInURL(widget.urlImage));
        downloader.requestDownload(
            task: taskInfo,
            fileName: appBloc.orientBloc.getNameFileInURL(widget.urlImage));
